...
impl ExecutionGraph {
    pub(crate) fn new(resolved: &GraphResourcesResolved, outputs: &[ResourceName]) -> Self {
        ...
        let outputs = outputs
            .iter()
            .filter_map(|res_name| {
                match resolved.name_lookup.get(res_name) {
                    None => {
        ...
        let exec_list = {
            ...
            for (res, pass) in last_use {
                pass_destroys.entry(pass).or_insert(HashSet::new()).insert(res);
            }


            pass_execs.into_iter()
                .map(|batch| {

                    use std::collections::BTreeSet;

                    let (creates, copies, deletes) = {
                        let all_creates = batch.iter()
                            .filter_map(|pass| resolved.pass_creates.get(pass))
                            .flatten();

                        let creates = all_creates.clone()
                            // We really only care about *new* things that are created.
                            // (no copies or moves)
                            .filter(|res| {
                                resolved.infos.contains_key(res)
                            })
                            .cloned()
                            .collect();
                        ...
                        let copies = all_creates
                            ...
                            .filter(|res| {
                                resolved.copies_from.contains_key(res)
                            })
                            .cloned()
                            .collect();

                        let deletes = batch.iter()
                            .filter_map(|pass| pass_destroys.get(pass))
                            .flatten()
                            // If a resource was created by moving the original
                            .filter_map(|res| {
                                resolved.moved_from(*res).or(Some(*res))
                            })
                            .filter(|res| {
                                !keep_list.contains(res)
                            })
